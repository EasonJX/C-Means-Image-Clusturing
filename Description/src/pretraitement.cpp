...
double image::d_Surface_contour() {
	vector<vector<Point>> tmp = this->vec_FindCt_externe();
	double area(0);
...
double image::d_Surface_hull() {
	...
	vector<vector<Point>> tmp = this->v_hull();
	double area(0);
	for (int i = 0; i < tmp.size(); i++) {
		area = area += contourArea(tmp[i]);
	}
...
double image::d_Perimetre_silhouette() {
	vector<vector<cv::Point>> tmp = this->vec_FindCt_externe();
	double pr(0);
	for (int i = 0; i < tmp.size(); i++) {
...
Moments image::m_Moment() {
	return moments(this->v_hull()[0], false);
}
...
double image::max_Dist_centroid_hull() {
	...
	resultat[0] = norm(this->p2f_Centroid_hull() - (Point2f)contour[0][0]);
	maxim = resultat[0];
	for (rsize_t i = 0; i < contour[0].size(); i++) {
		resultat[i] = norm(this->p2f_Centroid_hull() - (Point2f)contour[0][i]);
		if (resultat[i] >= maxim) {
			maxim = resultat[i];
		}
	}
...
double image::min_Dist_centroid_contour() {
	vector<vector<cv::Point>> contour = this->vec_FindCt_externe();
	Point2f dist;
	const rsize_t n = contour[0].size();
	double *resultat = new double[n];
	double  minim(0);
...
double image::somme_distance_centroid_contour(vector<vector<cv::Point>> contour) {
	...
	for (rsize_t i = 0; i < contour[0].size(); i++) {
		...
		distence = distence +norm(this->p2f_Centroid_hull() - (Point2f)contour[0][i]);
